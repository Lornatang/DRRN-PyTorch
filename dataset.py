...
class TrainValidImageDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        lr_image = cv2.imread(self.lr_image_file_name[batch_index], cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.
        hr_image = cv2.imread(self.hr_image_file_name[batch_index], cv2.IMREAD_UNCHANGED).astype(np.float32) / 255.

        if self.mode == "Train":
            # Data augment
            lr_image, hr_image = imgproc.random_crop(lr_image, hr_image, self.image_size)
            ...
            lr_image, hr_image = imgproc.center_crop(lr_image, hr_image, self.image_size)
        else:
